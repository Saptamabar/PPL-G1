...
<div class="container mx-auto px-2 sm:px-6">
    ...
    <div class="mb-12">
        ...
        <div class="bg-white shadow-md rounded overflow-x-auto mb-8">
            <table class="min-w-full">
                <thead>
                    <tr class="bg-gray-200 text-gray-600 uppercase text-xs sm:text-sm leading-normal">
                        ...
                        <th class="py-3 px-2 sm:px-6 text-left">Nama</th>
                        <th class="py-3 px-2 sm:px-6 text-left">Jenis</th>
                        <th class="py-3 px-2 sm:px-6 text-left">Jumlah</th>
                ...
                <tbody class="text-gray-600 text-xs sm:text-sm">
                    ...
                    <tr class="border-b border-gray-200 hover:bg-gray-100">
                        <td class="py-3 px-2 sm:px-6 text-left">{{ $item->id }}</td>
                        <td class="py-3 px-2 sm:px-6 text-left">{{ $item->nama }}</td>
                        <td class="py-3 px-2 sm:px-6 text-left">{{ $item->jenis }}</td>
                        <td class="py-3 px-2 sm:px-6 text-left">{{ $item->jumlah }}</td>
                        <td class="py-3 px-2 sm:px-6 text-center">
                            <div class="flex item-center justify-center space-x-1 sm:space-x-2">
                                <a href="{{ route('inventaris-habis.show', $item) }}" class="text-blue-500 hover:text-blue-700" title="Lihat">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 sm:h-5 sm:w-5" viewBox="0 0 20 20" fill="currentColor">
                                        <path d="M10 12a2 2 0 100-4 2 2 0 000 4z" />
                                        <path fill-rule="evenodd" d="M.458 10C1.732 5.943 5.522 3 10 3s8.268 2.943 9.542 7c-1.274 4.057-5.064 7-9.542 7S1.732 14.057.458 10zM14 10a4 4 0 11-8 0 4 4 0 018 0z" clip-rule="evenodd" />
                                    </svg>
                                </a>
                                <a href="{{ route('inventaris-habis.edit', $item) }}" class="text-green-500 hover:text-green-700" title="Edit">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 sm:h-5 sm:w-5" viewBox="0 0 20 20" fill="currentColor">
                                        <path d="M13.586 3.586a2 2 0 112.828 2.828l-.793.793-2.828-2.828.793-.793zM11.379 5.793L3 14.172V17h2.828l8.38-8.379-2.83-2.828z" />
                                    </svg>
                                </a>
                                <form action="{{ route('inventaris-habis.destroy', $item) }}" method="POST" class="inline">
                                    @csrf
                                    @method('DELETE')
    ...
    <!-- Inventaris Tidak Habis Section -->
    <div class="mb-12">
        <div class="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
            ...
            <table class="min-w-full">
                <thead>
                    <tr class="bg-gray-200 text-gray-600 uppercase text-xs sm:text-sm leading-normal">
                        <th class="py-3 px-2 sm:px-6 text-left">ID</th>
                        <th class="py-3 px-2 sm:px-6 text-left">Nama</th>
                        <th class="py-3 px-2 sm:px-6 text-left">Kode</th>
                        <th class="py-3 px-2 sm:px-6 text-center">Aksi</th>
                    </tr>
                </thead>
                <tbody class="text-gray-600 text-xs sm:text-sm">
                    ...
                    <tr class="border-b border-gray-200 hover:bg-gray-100">
                        <td class="py-3 px-2 sm:px-6 text-left">{{ $item->id }}</td>
                        <td class="py-3 px-2 sm:px-6 text-left">{{ $item->nama }}</td>
                        <td class="py-3 px-2 sm:px-6 text-left">{{ $item->kode }}</td>
                        <td class="py-3 px-2 sm:px-6 text-center">
                            <div class="flex item-center justify-center space-x-1 sm:space-x-2">
                                <a href="{{ route('inventaris-tak-habis.show', $item) }}" class="text-blue-500 hover:text-blue-700" title="Lihat">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 sm:h-5 sm:w-5" viewBox="0 0 20 20" fill="currentColor">
                                        <path d="M10 12a2 2 0 100-4 2 2 0 000 4z" />
                                        <path fill-rule="evenodd" d="M.458 10C1.732 5.943 5.522 3 10 3s8.268 2.943 9.542 7c-1.274 4.057-5.064 7-9.542 7S1.732 14.057.458 10zM14 10a4 4 0 11-8 0 4 4 0 018 0z" clip-rule="evenodd" />
                                    </svg>
                                </a>
                                <a href="{{ route('inventaris-tak-habis.edit', $item) }}" class="text-green-500 hover:text-green-700" title="Edit">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 sm:h-5 sm:w-5" viewBox="0 0 20 20" fill="currentColor">
                                        <path d="M13.586 3.586a2 2 0 112.828 2.828l-.793.793-2.828-2.828.793-.793zM11.379 5.793L3 14.172V17h2.828l8.38-8.379-2.83-2.828z" />
                                    </svg>
                                </a>
                                <form action="{{ route('inventaris-tak-habis.destroy', $item) }}" method="POST" class="inline">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="text-red-500 hover:text-red-700" title="Hapus" onclick="return confirm('Apakah Anda yakin ingin menghapus data ini?')">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 sm:h-5 sm:w-5" viewBox="0 0 20 20" fill="currentColor">
